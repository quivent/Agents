...
import { getBullshitardContent, getContentBySlug, estimateReadingTime } from '@/lib/content';
...
export async function generateStaticParams() {
  const content = getBullshitardContent();
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: `${content.metadata.title} - Bullshitard`,
    description: content.metadata.description || 'A masterpiece of technical fabrication',
  };
}

export default async function BullshitardContentPage({ params }: Props) {
  const { slug } = await params;
  const content = getContentBySlug(slug);
    ...
    <Layout>
      <div className="space-y-8">
        ...
        <TerminalWindow title={`${content.metadata.title.toLowerCase().replace(/\s+/g, '_')}.md`}>
          <CommandLine 
            command={`cat /bullshit_archive/${content.metadata.title.toLowerCase().replace(/\s+/g, '_')}.md`}
            output="Loading fabricated expertise..."
            delay={20}
          />
